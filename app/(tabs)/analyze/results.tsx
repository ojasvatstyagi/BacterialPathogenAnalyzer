...
export default function ResultsScreen() {
  ...
  const autoSaveToHistory = async (result: PredictionResponse) => {
    try {
      const { error: insertError } = await supabase.from('analyses').insert({
        ...
        image_url: imagePath || imageUri, // Prefer path, fallback to URI if missing (legacy)
  ...
  if (error || !prediction) {
    return (
      ...
      >
        <View
          style={[styles.errorCircle, { backgroundColor: colors.error + '15' }]}
        >
      ...
      >
        <Animated.View style={{ opacity: fadeAnim, paddingBottom: 40 }}>
          ...
          <View
            style={[
              styles.resultCard,
              {
                ...
                borderTopColor: isBpseudo ? colors.error : colors.success,
              },
            ]}
          >
          ...
          <View style={styles.detailsGrid}>
            <View
              style={[styles.detailItem, { backgroundColor: colors.surface }]}
            >
